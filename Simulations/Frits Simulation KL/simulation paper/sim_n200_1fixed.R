...
fix1 <-read.table(file="highH_init1.txt", sep="")
fix2 <-read.table(file="highH_init2.txt", sep="")
fix3 <-read.table(file="highH_init3.txt", sep="")
fix4 <-read.table(file="highH_init4.txt", sep="")
fix5 <-read.table(file="highH_init5.txt", sep="")
fix6 <-read.table(file="highH_init6.txt", sep="")
fix7 <-read.table(file="highH_init7.txt", sep="")
fix8 <-read.table(file="highH_init8.txt", sep="")
fix9 <-read.table(file="highH_init9.txt", sep="")
fix10<-read.table(file="highH_init10.txt", sep="")
fix11<-read.table(file="highH_init11.txt", sep="")
fix12<-read.table(file="highH_init12.txt", sep="")  
...
for (i in 1: iterations){
  ...
  for (r in 1: Nresp){
    ...
    #initial design (12 different initial sets)
          if( 0 <  r & r < 17){design_I<-fix1  #16
    }else if(17 <= r & r < 34){design_I<-fix2  #17
    }else if(34 <= r & r < 50){design_I<-fix3  #16
    }else if(50 <= r & r < 67){design_I<-fix4  #...
    }else if(67 <= r & r < 83){design_I<-fix5
    }else if(83 <= r & r < 100){design_I<-fix6
    }else if(100 <= r & r < 116){design_I<-fix7
    }else if(116 <= r & r < 133){design_I<-fix8
    }else if(133 <= r & r < 149){design_I<-fix9
    }else if(149 <= r & r < 166){design_I<-fix10
    }else if(166 <= r & r < 182){design_I<-fix11
    }else if(182 <= r & r < 201) {design_I<-fix12} #19
...
#write to excel file
write.table(X, file="X_sim19new.txt", quote = FALSE, sep=" ", row.names = FALSE)
write.table(D, file="Y_sim19new.txt", quote = FALSE, sep=" ", row.names = FALSE)
