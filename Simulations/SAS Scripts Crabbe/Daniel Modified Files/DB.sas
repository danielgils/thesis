/*
1) Construction of individualized choice designs for the mixed logit choice model with the DB criterion.
...
denlikeli[ndr,]=prob2(y,Xorg,prbeta[,ndr]);
...
nrt=(dr-1)*&df+1:dr*&df;
b=prbeta[,dr];
call infologit(Xorg,b,cset,info1);
infomat[nrt,]=info1;
end;
...
dmin=0;
	...
	subinfo=infomat[nrt,];
	b1=prbeta[,dr];
	call infologit2(X,b1,info2);
	dmin=dmin+det(info2+subinfo+invcovar)##(-1/&df)*weight[dr,];
	end;
if dmin<dsmall then;
do;
dsmall=dmin;
...
meanRMSE = RMSE[:,];
print meanRMSE;
create RMSE from RMSE; append from RMSE;
create mu from mu_rep_all; append from mu_rep_all;
create sigma from sigma_rep_all; append from sigma_rep_all;
create beta from beta_rep_all; append from beta_rep_all;
