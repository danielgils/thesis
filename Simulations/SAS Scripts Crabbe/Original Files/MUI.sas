/*
1) Construction of individualized choice designs for the mixed logit choice model with the MUI criterion.
...
do u=1 to nsetcand;
	...
	X=holddesign[l,];
		...
		klprior = klprior + ( logelemfin[klelem,] - (log(elemfin[klelem,]) * elemfin[klelem,]) );
		end;
...
do uu=1 to nsetcand;
	...
	X=holddesign[ij,];
		...
		klinfo = 0;
		do klelem = 1 to &nalts;
		klinfo = klinfo + ( logelemfin[klelem,] - (log(elemfin[klelem,]) * elemfin[klelem,]) );
		end;
