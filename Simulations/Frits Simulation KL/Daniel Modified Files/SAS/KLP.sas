...
/*
Construction of individualized tests for the mixed logit choice model with the KLP criterion.
*/

libname klchoice 'C:\Users\danie\Documents\Daniel Gil\KULeuven\Stage 2\Thesis\Simulations\Frits Simulation KL\Daniel Modified Files\SAS';
...
/*
proc plan ordered;
factors x1=2 x2=3 x3=2 x4=3/noprint;
output out=candidate;
run;
proc transreg design data=candidate;
model class(x1 x2 x3 x4/effects);
output out=candid;
run;
*/
...
/*
proc plan ordered;
factors x1=4 x2=2 x3=2 x4=2/noprint;
output out=candidate;
run;
proc transreg design data=candidate;
model class(x1 x2 x3 x4/effects);
...
/*
proc plan ordered;
factors x1=4 x2=2 x3=2 x4=2 x5=2 x6=2/noprint;
output out=candidate;
run;
proc transreg design data=candidate;
model class(x1 x2 x3 x4 x5 x6/effects);
output out=candid;
run;
*/
...
/*
proc plan ordered;
factors x1=3 x2=2 x3=3/noprint;
output out=candidate;
run;
proc transreg design data=candidate;
model class(x1 x2 x3/effects);
...
/*
proc plan ordered;
factors x1=3 x2=2 x3=2 x4=2 x5=2/noprint;
output out=candidate;
run;
proc transreg design data=candidate;
model class(x1 x2 x3 x4 x5/effects);
output out=candid;
run;
data candid; set candid;
if x11=-1 then x11=0;
...
use klchoice.betatrue1_nondiag; read all into truebeta; * Dan: Betatrue is the true betas for each of the 50 respondents;
/*
use klchoice.initialsets_KLP; read all into initialsets;
*/

use klchoice.mutrue1_nondiag; read all into pmean; * Dan: Mutru1 is the true mean of the prior distribution;
use klchoice.sigmatrue1_nondiag; read all into pcov; * Dan: Sigmatrue1 is the true variance (or sd, not sure) of the prior dist.;

* Dan: candidat just select the columns from the candidate set (candid) that are represented with effects coding;
...
do i = 1 to nrow(candidat)-1;
	do j = 1 to nrow(candidat)-i;
   		do k = 1 to nrow(candidat)-i-j;
    	holddesignfull = holddesignfull // candidat[i, ];
    	holddesignfull = holddesignfull // candidat[i + j, ];
		holddesignfull = holddesignfull // candidat[i + j + k, ];
		end; 
	end;
end;
*/
...
do rep=1 to &nrep; 
	...
	Xmat=j(obs,&df,0);
	ymat=j(&nres,&tset,0);
	use klchoice.latfull_6par; read all into zfull;
...
use klchoice.betatrue1_nondiag; read all into betatrue;
use klchoice.mutrue1_nondiag; read all into mutrue;
use klchoice.sigmatrue1_nondiag; read all into sigmatrue;
...
meanRMSE = RMSE[:,];
print meanRMSE;
create klchoice.RMSE_KLP_1nond100 from RMSE; append from RMSE;

create klchoice.mu_rep_all_KLP_1nond100 from mu_rep_all; append from mu_rep_all;
create klchoice.sigma_rep_all_KLP_1nond100 from sigma_rep_all; append from sigma_rep_all;
create klchoice.beta_rep_all_KLP_1nond100 from beta_rep_all; append from beta_rep_all;
...
/*
proc iml;
use klchoice.mudraw; read all into draw;
draw = draw`;
create draw from draw; append from draw;
quit;
symbol1 interpol=join; 
data draw; set draw; 
n=_N_;
run;
proc gplot data=draw; 
plot COL1*n COL2*n COL3*n COL4*n COL5*n COL6*n; 
run;
quit;
*/
